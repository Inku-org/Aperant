...
import { LinearIssues } from "./components/LinearIssues";
...
export function App() {
  ...
  const [skippedInitProjectId, setSkippedInitProjectId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (projects.length > 0) {
      ...
      if (openProjectIds.length === 0) {
        ...
        console.warn(
          "[App] No tabs persisted, opening project:",
          projectToOpen,
        );
  ...
  useEffect(() => {
    ...
    const hasOAuthConfigured = claudeProfiles.some(
      (p) => p.oauthToken || (p.isDefault && p.configDir),
    );
  ...
  useEffect(() => {
    const syncSpellCheck = async () => {
      try {
        const result = await window.electronAPI.setSpellCheckLanguages(
          i18n.language,
        );
        if (!result.success) {
          console.warn(
  ...
  useEffect(() => {
    ...
    if (
      ...
      skippedInitProjectId !== selectedProject.id
    ) {
  ...
  useEffect(() => {
    ...
    const colorTheme: ColorTheme = validThemeIds.includes(
  ...
  useEffect(() => {
    ...
    const reviewReasonChanged =
      selectedTask.reviewReason !== updatedTask.reviewReason;
    const logsChanged =
  ...
  const handleInitialize = async () => {
    ...
    console.warn(
      "[InitDialog] Starting initialization for project:",
      projectId,
    );
    ...
    try {
      ...
      if (result?.success) {
        ...
        const errorMessage =
          result?.error || "Failed to initialize Aperant. Please try again.";
      ...
      <TooltipProvider>
        ...
        <div className="flex h-screen bg-background">
            ...
            <main className="flex-1 overflow-hidden">
              {selectedProject ? (
                <>
                  ...
                  <div
                    className={activeView === "terminals" ? "h-full" : "hidden"}
                  >
                    <TerminalGrid
                  ...
                  {activeView === "changelog" &&
                    (activeProjectId || selectedProjectId) && <Changelog />}
          ...
          <Dialog
            open={showInitDialog}
            onOpenChange={(open) => {
              console.warn("[InitDialog] onOpenChange called", {
                open,
