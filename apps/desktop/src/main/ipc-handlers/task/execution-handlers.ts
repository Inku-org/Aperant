...
} from "../../utils/git-isolation";
import { cancelFallbackTimer } from "../agent-events-handlers";
import { readSettingsFile } from "../../settings-utils";
import type { ProviderAccount } from "../../../shared/types/provider-account";
import { syncTaskStatusToLinearInBackground } from "../linear/sync-handlers";
...
export function registerTaskExecutionHandlers(
  ...
  ipcMain.on(
    ...
    async (_, taskId: string, _options?: TaskStartOptions) => {
      ...
      if (!task || !foundProject) {
        console.warn(
          "[TASK_START] Task or project not found for taskId:",
          taskId,
        );
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
      ...
      const project =
        task.projectId && task.projectId !== foundProject.id
      ...
      if (!gitStatus.isGitRepo) {
        console.warn(
          "[TASK_START] Project is not a git repository:",
          project.path,
        );
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
      ...
      if (!gitStatus.hasCommits) {
        console.warn(
          "[TASK_START] Git repository has no commits:",
          project.path,
        );
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
      ...
      if (!profileManager.hasValidAuth() && !hasAnyProviderAccount()) {
        console.warn(
          "[TASK_START] No valid authentication for active profile or provider accounts",
        );
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
      ...
      if (currentXState === "plan_review") {
        ...
        console.warn(
          "[TASK_START] XState: plan_review -> coding via PLAN_APPROVED",
        );
        ...
        console.warn(
          "[TASK_START] XState: error with no plan subtasks -> planning via PLANNING_STARTED",
        );
        taskStateManager.handleUiEvent(
          taskId,
        ...
        console.warn(
          "[TASK_START] XState in unexpected state:",
          currentXState,
          "- sending PLANNING_STARTED",
        );
        taskStateManager.handleUiEvent(
          taskId,
          { type: "PLANNING_STARTED" },
        ...
        console.warn(
          "[TASK_START] No XState actor, task data: plan_review -> coding via PLAN_APPROVED",
        );
        taskStateManager.handleUiEvent(
          taskId,
          { type: "PLAN_APPROVED" },
        ...
        // Uses planHasSubtasks from implementation_plan.json (more reliable than task.subtasks.length).
        console.warn(
          "[TASK_START] No XState actor, error with no plan subtasks -> planning via PLANNING_STARTED",
        );
        taskStateManager.handleUiEvent(
          taskId,
          { type: "PLANNING_STARTED" },
        ...
        console.warn(
          "[TASK_START] No XState actor, task data:",
          task.status,
          "-> coding via USER_RESUMED",
        );
        taskStateManager.handleUiEvent(
      ...
      if (resetResult.success && resetResult.resetCount > 0) {
        console.warn(
          `[TASK_START] Reset ${resetResult.resetCount} stuck subtask(s) before starting`,
        );
      }

      // Start file watcher for this task
      // Use worktree path if it exists, since the backend writes implementation_plan.json there
      const watchSpecDir = getSpecDirForWatcher(
        project.path,
        specsBaseDir,
        task.specId,
      );
      fileWatcher.watch(taskId, watchSpecDir).catch((err) => {
        console.error(
          `[TASK_START] Failed to watch spec dir for ${taskId}:`,
          err,
        );
      });
      ...
      if (needsSpecCreation) {
        ...
        console.warn(
          "[TASK_START] Starting task execution (no valid subtasks in plan) for:",
          task.specId,
        );
      ...
      if (approved) {
        ...
        if (hasWorktree) {
          ...
          if (cleanResult.status === 0) {
            console.log(
              "[TASK_REVIEW] Cleaned untracked files in main (excluding .auto-claude)",
            );
          }
    ...
    > => {
      ...
      if (status === "done") {
        ...
        if (hasWorktree) {
          if (options?.keepWorktree) {
            ...
            console.warn(
              `[TASK_UPDATE_STATUS] Marking task ${taskId} as done while keeping worktree at ${worktreePath}`,
            );
          } else if (options?.forceCleanup) {
            // User confirmed cleanup - delete worktree and branch
            console.warn(
              `[TASK_UPDATE_STATUS] Cleaning up worktree for task ${taskId} (user confirmed)`,
            );
            try {
              ...
              const { branch, usingFallback: usingFallbackBranch } =
                detectWorktreeBranch(worktreePath, task.specId, {
                  timeout: 30000,
              ...
              console.warn(
                `[TASK_UPDATE_STATUS] Worktree removed: ${worktreePath}`,
              );
              ...
              try {
                ...
                if (usingFallbackBranch) {
                  ...
                  console.warn(
                    `[TASK_UPDATE_STATUS] Could not delete branch ${branch} using fallback pattern. Actual branch may still exist and need manual cleanup.`,
                    branchDeleteError,
                  );
                } else {
                  console.warn(
              ...
              console.warn(
                `[TASK_UPDATE_STATUS] Worktree cleanup completed successfully`,
              );
            } catch (cleanupError) {
              console.error(
                `[TASK_UPDATE_STATUS] Failed to cleanup worktree:`,
                cleanupError,
              );
              return {
                success: false,
            ...
            console.warn(
              `[TASK_UPDATE_STATUS] Worktree exists for task ${taskId}. Requesting user confirmation.`,
            );
            return {
              success: false,
              worktreeExists: true,
              ...
              error:
                "A worktree still exists for this task. Would you like to delete it and mark the task as complete?",
            };
          }
        } else {
      ...
      try {
        ...
        if (status === "in_progress" && !agentManager.isRunning(taskId)) {
          ...
          if (!profileManager.hasValidAuth() && !hasAnyProviderAccount()) {
            console.warn(
              "[TASK_UPDATE_STATUS] No valid authentication for active profile or provider accounts",
            );
            if (mainWindow) {
              mainWindow.webContents.send(
                IPC_CHANNELS.TASK_ERROR,
          ...
          if (resetResult.success && resetResult.resetCount > 0) {
            console.warn(
              `[TASK_UPDATE_STATUS] Reset ${resetResult.resetCount} stuck subtask(s) before starting`,
            );
          }
          ...
          fileWatcher.watch(taskId, watchSpecDir).catch((err) => {
            console.error(
              `[TASK_UPDATE_STATUS] Failed to watch spec dir for ${taskId}:`,
              err,
            );
          });
          ...
          const needsSpecCreation = !hasSpec;
          // FIX (#1562): Check actual plan file for subtasks, not just task.subtasks.length
          const updatePlanFilePath = path.join(
            specDir,
            AUTO_BUILD_PATHS.IMPLEMENTATION_PLAN,
          );
        ...
        // Sync status to Linear in background (fire-and-forget)
        syncTaskStatusToLinearInBackground(project.id, task.specId, status);
  ...
  ipcMain.handle(
    ...
    async (_, taskId: string): Promise<IPCResult> => {
      ...
      try {
        ...
        console.error(
          "[TASK_RESUME_PAUSED] Failed to write RESUME file:",
          error,
        );
        return {
          success: false,
  ...
  ipcMain.handle(
    ...
    > => {
      ...
      console.log(
        `[Recovery] Writing to plan file at: ${planPath} (task location: ${task.location || "main"})`,
      );
      ...
      if (
        mainSpecDir !== specDir &&
        existsSync(path.join(mainSpecDir, AUTO_BUILD_PATHS.IMPLEMENTATION_PLAN))
      ) {
        planPathsToUpdate.push(
          path.join(mainSpecDir, AUTO_BUILD_PATHS.IMPLEMENTATION_PLAN),
        );
      }
      if (
        worktreeSpecDir &&
      ...
      try {
        ...
        if (planContent) {
          try {
            ...
            console.error(
              "[Recovery] Failed to parse plan file as JSON:",
              parseError,
            );
            return {
              success: false,
              error:
                "Plan file contains invalid JSON. The file may be corrupted.",
            };
          }
        }
        ...
        if (plan) {
          ...
          if (allCompleted) {
            ...
            for (const pathToUpdate of planPathsToUpdate) {
              try {
                ...
                console.log(
                  `[Recovery] Successfully wrote to: ${pathToUpdate}`,
                );
          ...
          for (const pathToUpdate of planPathsToUpdate) {
            try {
              const resetResult = await resetStuckSubtasks(
                pathToUpdate,
                project.id,
              );
              if (resetResult.success) {
                ...
                if (resetResult.resetCount > 0) {
                  console.log(
                    `[Recovery] Reset ${resetResult.resetCount} stuck subtask(s) in: ${pathToUpdate}`,
                  );
                }
              } else {
                resetFailedCount++;
              ...
              resetFailedCount++;
              console.error(
                `[Recovery] Failed to reset stuck subtasks at ${pathToUpdate}:`,
                resetError,
              );
            }
          }
          ...
          if (resetFailedCount > 0) {
            console.warn(
              `[Recovery] Partial reset: ${totalResetCount} subtask(s) reset, but ${resetFailedCount} location(s) failed`,
            );
          }

          console.log(
            `[Recovery] Total ${totalResetCount} subtask(s) reset across all locations`,
          );
          ...
          if (worktreeSpecDir && worktreeSpecDir !== specDir)
            specDirsToClean.add(worktreeSpecDir);
          ...
          for (const dir of specDirsToClean) {
            ...
            try {
              ...
              console.log(
                `[Recovery] Cleared attempt_history.json at: ${dir} (reset ${stuckIds.size} stuck entries)`,
              );
            } catch (historyErr) {
              console.warn(
                `[Recovery] Could not parse attempt_history at ${dir}:`,
                historyErr,
              );
            }
          }
        ...
        if (autoRestart) {
          ...
          if (
            !gitStatusForRestart.isGitRepo ||
            !gitStatusForRestart.hasCommits
          ) {
            console.warn(
              "[Recovery] Git check failed, cannot auto-restart task",
            );
          ...
          if (!profileManager.hasValidAuth() && !hasAnyProviderAccount()) {
            console.warn(
              "[Recovery] Auth check failed, cannot auto-restart task",
            );
            ...
            return {
              ...
              data: {
                ...
                newStatus,
                message:
                  "Task recovered but cannot restart: authentication required. Please add an account in Settings > Accounts.",
                autoRestarted: false,
              },
            };
          }
          ...
          try {
            ...
            if (plan) {
              ...
              for (const pathToUpdate of planPathsToUpdate) {
                try {
                  ...
                  console.log(
                    `[Recovery] Wrote restart status to: ${pathToUpdate}`,
                  );
                } catch (writeError) {
                  console.error(
            ...
            fileWatcher.watch(taskId, watchSpecDir).catch((err) => {
              console.error(
                `[Recovery] Failed to watch spec dir for ${taskId}:`,
                err,
              );
            });
            ...
            if (needsSpecCreation) {
              ...
              const taskDescription = task.description || task.title;
              console.warn(
                `[Recovery] Starting spec creation for: ${task.specId}`,
              );
              agentManager.startSpecCreation(
                taskId,
                project.path,
              ...
              console.warn(
                `[Recovery] Starting task execution for: ${task.specId}`,
              );
              agentManager.startTaskExecution(
                taskId,
                project.path,
            ...
            console.error(
              "Failed to auto-restart task after recovery:",
              restartError,
            );
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : "Failed to recover task",
        };
      }
    },
  );
