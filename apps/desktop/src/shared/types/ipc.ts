...
export interface ElectronAPI {
  ...
  checkWorktreeChanges: (taskId: string) => Promise<
    IPCResult<{
      hasChanges: boolean;
      worktreePath?: string;
  ...
  onUsageUpdated: (
    callback: (usage: ClaudeUsageSnapshot) => void,
  ) => () => void;
  ...
  getProviderAccounts: () => Promise<
    IPCResult<{ accounts: ProviderAccount[] }>
  >;
  saveProviderAccount: (
  ...
  testProviderConnection: (
    provider: string,
    config: { apiKey?: string; baseUrl?: string; region?: string },
  ...
  codexAuthStatus: () => Promise<{
    success: boolean;
    data?: { isAuthenticated: boolean; expiresAt?: number };
    error?: string;
  }>;
  ...
  getMemoryStatus: (
    projectId: string,
  ) => Promise<IPCResult<MemorySystemStatus>>;
  searchMemories: (
    projectId: string,
    query: string,
  ...
  getRecentMemories: (
    projectId: string,
    limit?: number,
  ) => Promise<IPCResult<RendererMemory[]>>;
  ...
  testMemoryConnection: (
    dbPath?: string,
    database?: string,
  ) => Promise<IPCResult<MemoryValidationResult>>;
  ...
  ) => Promise<IPCResult<void>>; // Async with progress events
  saveChangelog: (
  ...
  deleteInsightsSessions: (
    projectId: string,
    sessionIds: string[],
  ) => Promise<IPCResult<{ deletedIds: string[]; failedIds: string[] }>>;
  archiveInsightsSession: (
    projectId: string,
    sessionId: string,
  ) => Promise<IPCResult>;
  archiveInsightsSessions: (
    projectId: string,
    sessionIds: string[],
  ) => Promise<IPCResult<{ archivedIds: string[]; failedIds: string[] }>>;
  unarchiveInsightsSession: (
    projectId: string,
    sessionId: string,
  ) => Promise<IPCResult>;
